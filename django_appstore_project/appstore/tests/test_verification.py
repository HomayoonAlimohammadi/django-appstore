...
class AdminVerificationTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.app1: App = App.objects.create(
            title="App 1", description="Description 1", price="1.99", owner=self.user, is_verified=False
        )
        self.app2: App = App.objects.create(
            title="App 2", description="Description 2", price="2.99", owner=self.user, is_verified=False
        )
